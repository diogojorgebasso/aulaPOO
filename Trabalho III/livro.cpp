...
public:
  Livro()
      : titulo(""), autor(""), isbn(""), editora(""), anoPublicacao(0),
        disponivel(true), preco(0), quantidadeEmEstoque(0) {}
  Livro(string titulo, string autor, string ISBN, string editora,
        int anoPublicacao, double preco, int quantidadeEmEstoque)
      : titulo(titulo), autor(autor), isbn(ISBN), editora(editora),
        anoPublicacao(anoPublicacao), disponivel(true), preco(preco),
  ...
  bool operator==(const Livro &book) const { return isbn == book.isbn; }
